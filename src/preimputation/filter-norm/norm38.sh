...
normalize_vcf() {
    ...
    bcftools +fixref "${input_vcf}" -Oz -o "chr${chr}_filtered.vcf.gz"  -- -f "${reference_genome}" -m top
    bcftools norm -f "${reference_genome}" -m -any "chr${chr}_filtered.vcf.gz" -O z -o "${norm_vcf}"
